...
def run_logistic_regression(hyperparameters):
    ...
    weights = numpy.random.rand(M+1).reshape(-1,1)
...
if __name__ == '__main__':
    ...
    for weight_decay in [0.3]:
        hyperparameters = {
                    ...
                    'weight_decay': weight_decay # related to standard deviation of weight prior 
                    }
        ...
        logging /= num_runs
        #plt.plot(range(num_iter),logging[:,4],label = "weight decay = " + str(weight_decay))
        plt.plot(range(num_iter),logging[:,1],label = "Training Cross Entropy, weight decay = "+str(weight_decay))
        plt.plot(range(num_iter),logging[:,3],label = "Validation Cross Entropy, weight decay = "+str(weight_decay))
