...
def AffineBackward(grad_y, x, w):
    ...
    ###########################
    grad_x = np.dot(grad_y, np.transpose(w))
    grad_w = np.dot(np.transpose(x), grad_y)
...
def Train(model, forward, backward, update, eps, momentum, num_epochs,
    ...
    for epoch in range(num_epochs):
        ...
        for step in range(num_steps):
            ...
            #Commented out for faster results, printing takes too long
...
#Repurposed to show first layer weights
def ShowWeights(means, number=0):
  ...
  plt.draw()
  plt.pause(0.0001)
  plt.savefig('NNFirstLayerWeights.png')
  raw_input('Press Enter.')
  
#Copied from kmeans.py
#Repurposed to show pictures of faces without a confident score
def ShowNotConfident(model):
    inputs_train, inputs_valid, inputs_test, target_train, target_valid, target_test = LoadData('../toronto_face.npz')
    count = 0
    for i in xrange(inputs_train.shape[0]):
        prediction = Softmax(NNForward(model, inputs_train[i:i+1])['y'])
        confidence = np.sum(prediction, axis=0)
        if np.max(confidence) < 0.5:
            print(confidence)
            plt.figure(count+2)
            plt.imshow(inputs_train[i,:].reshape(48,48), cmap=plt.cm.gray)
            plt.show()
            count+=1
            if count == 6:
                break
            
def main(num_hiddens, eps, momentum, num_epochs, batch_size):
    """Trains a NN."""
    ...
    #Uncomment for Q3.4
    #ShowWeights(model['W1'])

    #Uncomment for Q3.5
    #ShowNotConfident(model)
...
if __name__ == '__main__':
    ...
    #Q3.1
    #main(num_hiddens, eps, momentum, num_epochs, batch_size)

    #Q3.2
    #main(num_hiddens, 0.001, momentum, num_epochs, batch_size)
    #main(num_hiddens, 0.01, momentum, num_epochs, batch_size)
    #main(num_hiddens, 0.1, momentum, num_epochs, batch_size)
    #main(num_hiddens, 0.2, momentum, num_epochs, batch_size)
    #main(num_hiddens, 1.0, momentum, num_epochs, batch_size)

    #main(num_hiddens, eps, 0.0, num_epochs, batch_size)
    #main(num_hiddens, eps, 0.2, num_epochs, batch_size)
    #main(num_hiddens, eps, 0.45, num_epochs, batch_size)

    #main(num_hiddens, eps, momentum, num_epochs, 1)
    #main(num_hiddens, eps, momentum, num_epochs, 250)
    #main(num_hiddens, eps, momentum, num_epochs, 500)
    #main(num_hiddens, eps, momentum, num_epochs, 750)
    #main(num_hiddens, eps, momentum, num_epochs, 1000)

    #Q3.3
    #main([2, 32], 0.01, momentum, num_epochs, batch_size)
    #main([50, 32], 0.01, momentum, num_epochs, batch_size)
    #main([100, 32], 0.01, momentum, num_epochs, batch_size)
    #main([16, 2], 0.01, momentum, num_epochs, batch_size)
    #main([16, 50], 0.01, momentum, num_epochs, batch_size)
    #main([16, 30], 0.01, momentum, num_epochs, batch_size)

    #Q3.4
    #main([4,8], eps, momentum, num_epochs, batch_size)
